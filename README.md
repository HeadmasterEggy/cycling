...
- **好跑指数** —— 自由通行耗时 ÷ 实际耗时,一个比值,没有权重。
  自由通行取 25.0 km/h(平路、不在路口、实测移动速度的 90 分位),所以 2.4 min/km 是理论下限,
  指数 60 就是「只跑出了自由流的六成」。
  旧版是四个归一化分项的加权和(速度 34% + 红灯 28% + 爬升 20% + 龟速 18%),数据够多之后
  暴露出两个毛病:
  一是**重复计算** —— 每公里耗时本身就包含了等红灯和龟速的时间,再当独立项加权等于把同一分钟
  算两三遍。实测分项之间纠缠得很厉害(pace × stopgo r=0.76,pace × lights r=0.64),
  最终指数和它自己的 pace 分项 r=0.87,等于把一个数字装扮成四个。
  二是**权重和物理不符** —— 爬升占 20%,但数据里的速度-坡度曲线说每爬升 1 米约多花 0.01 分钟,
  一个 12 m/km 的坡区每公里只损失 0.12 分钟,而红灯要吃掉 0.4–1.7 分钟。红灯的代价是坡的十倍,
  旧指数说的正相反。
  所以现在分数就是那个比值,四个数字改成对差距的**分解**:每一分钟损失只归因一次 ——
  等红灯 / 短停 / 龟速 / 巡航偏慢,坡度作为「巡航偏慢」里的一部分单独标出来。
  分项加起来等于实测总耗时,而不是凑成 100。
- **样本不够的区直接不上榜** —— 门槛不是里程,而是**分数本身的精度**。
  每个区按班次做 600 次 bootstrap,算出 90% 置信区间;区间宽超过 12 分的区不在页面上出现。
  旧门槛(5 km + 4 个班次)听起来够了,实测区间宽 ±18 分 —— 一个报 55 分的区真值可能在 46 到 64
  之间,那张表里的名次就没有意义。现在 58 个去过的区里有 18 个达标,中位区间宽 7.5 分。
  未达标的区,估计值在数据层面就被置空(不只是界面上灰掉),计数类字段(去过几次、多少单)保留 ——
  那是观测,不是估计。
- **值得去** —— 单位是「活/小时」,不再是对最好那个区取比值的 0–100 分。
  = 收缩后的活/小时 × (本区好跑 / 全城好跑中位)。样本不够的区不给分,也不参与排名
  (门槛:`RANK_MIN_KM` 公里 + `RANK_MIN_SHIFTS` 个班次)。
- **落点之后** —— 送完到下一次取餐之间那段路没人付钱,占总里程 41%。
  这一段按方向分三种:**往回骑**(下一次取餐比这次送达更靠近取餐重心,至少近 200 米)、
  **就地续上**、**继续往外**。实测 35% 是往回骑,中位 1.94 km / 9.8 分钟,
  是就地续单(0.55 km / 3.5 分钟)的三倍多。
  所以对时薪伤害最大的不是「送得远」,而是**落在没有餐厅的地方** —— 那里不会有新单找上你,
  只能自己骑回去。各区据此标成「接得上」或「要往回骑」。
- **路段** —— 常骑的 60 米格子连成链。单个格子不成路,一串才是。「最难走」按每趟多花的
  时间排序,不是累计 —— 否则排出来的只是最长最常走的那几条。

### 接单区还是送达区

一个区很少两头都好。`build_transitions()` 把每个班次拆成「取餐 → 送达 → 骑去下一次取餐」
的链条,分别记两种角色:

- **作为接单区** —— 取餐次数、餐厅让你等多久、接了之后要骑多远才送到。送得越近,
  同样时间里能跑的单越多。
- **作为送达区** —— 送完之后到下一次取餐的时间和距离(空驶),以及下一单还在不在同一个区。

**空驶是最大的一笔隐形成本**:全部里程里 41% 花在「送完了、还没接到下一单」的路上。
中位每次 1.2 km / 5.7 分钟,37% 的情况下一单仍在同一个 suburb。

两种不同的坏要分开看:空驶时间长、距离也长(Surry Hills、Pyrmont)是真的在跑远路;
时间长但距离很短(Haymarket 11.9 分钟只骑了 0.38 km)是停在原地等派单 —— 后者换位置
没用,得换时段。页面上会自动标出来。

只统计相邻两次作业间隔 ≤ 45 分钟的段落 —— 更长的是休息或跨城转场,不是接单间隙。
一共只有 76 段空驶,摊到各区每区几段,少于 3 段的不列、少于 4 段的标灰。

### 判得准不准

三个平台里有一个（Uber）能导出真实订单记录,`analyze_delivery.py --uber` 拿它当答案卷,
把分类器的实测分数写进 `validation`,页面上直接印出来。

- **时间锚点** —— 订单记录里的时间戳是<em>取餐</em>时刻,不是送达。用「时间戳 + 时长」对齐,
  送达停留的中位偏差只有几十秒;直接用时间戳会差出几百秒。能对得这么齐,本身就说明停留
  检测抓到的是真事件,不是把噪声拟合成了图案。
- **只报召回率,不报准确率** —— 这份记录只是三个平台里的一个(另外两个是熊猫外卖和
  DoorDash,没有记录可对)。推断出的送达里只有约四分之一能在这份记录里找到,剩下的绝大
  多数是另外两个平台的单,不是误判。准确率需要每个平台的记录才能算,拿一个平台的去算
  等于撒谎,所以页面不算。
- **没录的时段先剔除** —— 约三分之一的订单发生在手表没在记录的时候,不可能匹配,已从
  分母里去掉。

`data/uber/uber_activity.psv` 是那份记录的导出(`时间戳|类型|金额|时长秒|距离km|取餐点|送达地址`)。
**它不在仓库里**,`data/` 整个目录都被 `.gitignore` 排除:送达地址那一列是客户的家庭住址,
而这个站是公开的。只有汇总分数会离开本地 —— 页面上不会出现任何一个客户地址。
