...
| `delivery.html` | `delivery` | 配送效率:停留分类(取餐/送达/等灯)、区域排行、该不该接单（Quest + $31.30 底薪对照）、可交互热点地图、路段好坏、时段规律、区域档案 |
...
两件事共同决定这一节:Quest 奖励,和法定底薪。

**Quest 是核对过的。**Uber 周结算单把收入拆成 Fare / Promotion (Quest) / Other / Tip 四行。
打开其中两周的结算单和本地导出逐行对:

| 结算周 | Quest(结算单) | Quest(日志) | Fare(结算单) | Fare(日志) |
|---|---|---|---|---|
| 2026-08-17 | $507.00 | **$507.00** | $364.95 | $368.12 |
| 2026-08-24 | $715.00 | **$715.00** | $517.66 | $520.72 |

Quest 一分不差。Fare 每周差 $3 上下,是跨周界的那一单(结算周从周一 04:00 起算)。
所以「Quest 是单独一笔、不含在单价里」是核对出来的,不是猜的。
日志里的 MISC 不是独立类别 —— 73 笔和 QUEST 金额相同、时间差十分钟内,是同一笔列了两次。

**Quest 按单发,不按钱发。**「跑满 N 单给 $X」,所以 `read_earnings()` 把每周的 Quest
总额平摊到那一周的订单数上。它占全部收入的 **49%**,最近两周每单 **$13.29**。
一笔 $500 的单笔付款没有摊进去 —— 单笔太大,不像跑单挣的。

**于是单价几乎不再决定时薪。**同样 143 单,换个排法就是两个结论:

| 按用时排 | 时薪 | | 按单价排 | 时薪 |
|---|---|---|---|---|
| <25 min | **$56.6** | | $0–6 | $33.9 |
| 25–32 | $43.0 | | $6–8 | $28.5 |
| 32–40 | $34.6 | | $8–11 | $33.0 |
| 40–55 | $24.5 | | $11–15 | $38.6 |
| 55+ | **$13.6** | | $15+ | $36.1 |

快慢差 **4.2 倍**,贵贱几乎一样。因为跑一单给一次 Quest,一单 $5 和一单 $15 收的是同一笔。
秩相关也是这个意思:用时 ρ +0.78,单价 ρ **+0.05**。
**所以派单页上该看的不是金额,是这单要花多久。**

**8 月 17 日那一周,每单 Quest 从 $4–6 跳到 $12.60。**同一周,
[Fair Work Commission 的最低标准令](https://www.fairwork.gov.au/about-us/workplace-laws/fair-work-commission-orders/minimum-standards-order-on-demand-delivery-workers)
生效:自行车/电动自行车 **$31.30/小时**,按 engaged time(接单到送达完成)计,
最长 21 天一个结算周期,平台低于下限要补足。同期 engaged 时薪从 $22.4 变成 **$40.6**,
越过了下限。量级不像巧合,但结算单上没有单独的补贴条目,所以页面只说两件事发生在同一周。

`engaged` 时间取「上一单交货那一刻」到送达,再扣掉停在既不是红灯也不是餐厅的地方
干等派单的时间(中位 0.5 分钟);中位每单 34.3 分钟。
计算器用 `engaged 分钟 ≈ 3.15 + 5.30 × 总公里`(R² 0.78),Quest 那一档可调,
默认放在最近两周的实际水平。

页面结构:一句话 + 四个时薪、一单的单价/Quest 堆叠条、两张互为对照的分档表、
三滑块计算器(散点横轴是 engaged 分钟,按是否越过 $31.30 着色)、每周 Quest 柱、
以及拼单/时段/落点三张小卡。

> 这一节改过两次,方向都变了。第一版只看单价,说「便宜的单该拒」;第二版加进底薪,
> 说「拒单等于扔工时」—— 那时还没把 Quest 算进收入,误以为在底薪之下。
> 核对完 Quest 才是现在这版。页面上写了这个过程。
