...
| `delivery.html` | `delivery` | 配送效率:停留分类(取餐/送达/等灯)、区域排行、该不该接单（含 $31.30 法定底薪对照）、可交互热点地图、路段好坏、时段规律、区域档案 |
| `dashboard.html` | `dashboard` | 配送作战台:同一份数据的单屏版本,八个面板一屏排完（含接单/底薪计算器）,点任一面板的区在所有面板同时高亮 |
...
2026 年 8 月 17 日,Fair Work Commission 的
[Interim On-Demand Delivery Employee-like Worker Minimum Standards Order](https://www.fairwork.gov.au/about-us/workplace-laws/fair-work-commission-orders/minimum-standards-order-on-demand-delivery-workers)
生效:自行车/电动自行车 **$31.30/小时**,摩托车 $31.50,汽车 $32,按 **engaged time**
(从接单到送达完成)计,最长 21 天一个结算周期,平台低于下限要补足。

这一条把整节的结论翻了过来,所以 `build_offers()` 是围绕它重写的。

**两个时钟。**「marginal」只算骑行(去餐厅 + 计价段),是接了才会付出、拒了就不会付出的时间;
「engaged」是法规的定义 —— 接单那一刻两份数据都没有,所以取<em>上一单交货的那一刻</em>,
再扣掉停在既不是红灯也不是餐厅的地方干等派单的时间(中位 0.5 分钟)。
中位每单 34.3 分钟 engaged。

**核心数字。**单价折成 engaged 时薪只有 **$14.8**,底薪是 **$31.30**,差 $16.5/小时;
143 单里 **94%** 给的钱比它自己占掉的 engaged 时间按底薪该值的钱还少。
中位一单:平台给 $7.31,同样 34.3 分钟按底薪值 $17.89 —— 时钟每分钟 $0.522,单价每分钟 $0.21。

**所以规则反了。**在底薪之下,多接一单 = 多一段 engaged 时间 = 多 $0.522/分钟,
跟这单给多少钱无关;拒单减少的是自己的工时。真正的敌人从「便宜的单」变成「没在单上的时间」——
而空驶恰恰不算 engaged,是底薪唯一没兜住的部分。上一版给的建议(「便宜的单该拒」)在页面里
明确标了是反的。

**旧规则降级、没有删。**底薪是整个周期结算的,一旦周期内已经在 $31.30 之上,补贴归零、
单价重新变成唯一收入,这时候才用得上「每骑一公里挣多少(含骑去餐厅的路)」那套分档 ——
它仍然是单调的,而且解释权全在最右边那列「先要骑多少去拿餐」。

两个拟合撑起计算器:

- `engaged 分钟 ≈ 3.15 + 5.30 × 总公里`(R² 0.78)—— 距离几乎就决定了时间。
- `金额 ≈ $3.05 + $0.47/km + $0.203/min`(R² 0.68)—— 时间那一项几乎是零,
  堵车不会多给钱,但会多给 engaged 时间。

**推广收入没有并进正文。**日志里除单价外还有 84 笔推广/调整合计 $2251
(QUEST 和 MISC 两个类别里有 73 笔金额相同、时间差十分钟内,按同一笔算)。
按每单摊 $11.43,这 143 单约对应 $1634,加上去 engaged 时薪是 $34.2/h —— 正好在底薪上方一点,
非常像「这里面就含着底薪补贴」。但无法确认它们是另给的还是已含在单价里,
`drivers.uber.com` 的 activity 和 tax summary 两个页面现在都返回 404,查不到分类。
所以正文每一个时薪都只算单价,这个数只出现在方法说明里。

页面上是:一句话结论 + 三个数(单价时薪 / 底薪 / 差额)、一单的两根对比条、
8 月 17 日前后两张互相矛盾的规则卡、一个三滑块计算器(散点按「在底薪之上/之下」着色)、
以及降级后的 $/km 分档。
