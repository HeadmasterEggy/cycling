...
| `delivery.html` | `delivery` | 配送效率:停留分类(取餐/送达/等灯)、区域排行、接单决策计算器、可交互热点地图、路段好坏、时段规律、区域档案 |
| `dashboard.html` | `dashboard` | 配送作战台:同一份数据的单屏版本,八个面板一屏排完(含接单计算器),点任一面板的区在所有面板同时高亮 |
...
派单页给三个数字:多少钱、多少公里、大概多久。三个都只描述<em>取到餐之后</em>的那一段。
`build_offers()` 把订单日志和码表轨迹对起来,补上日志看不见的那一半 —— 从上一单交货
到这一单在餐厅取到餐的路 —— 再把结果变成一条八秒钟内能用的规则。

**两个分母,回答两个问题。**「实际时薪」的分母是<em>接了才会付出</em>的时间(骑去餐厅 + 计价段),
因为决定该不该接的是这个;「真实时薪」再加上中间站着的时间,那才是一小时到手的钱。
中位分别是 $19.2/h 和 $15.9/h,而派单页上写着 $27.6/h。

**决定时薪的不是钱。**各项与实际时薪的秩相关:取餐点有多远 +0.78,这一单给多少钱 +0.28,
「每公里多少钱(只算送餐段)」—— 大部分人真正在用的那个标准 —— 只有 +0.21。
接近距离本身就在时薪的分母里,所以它排前面有一部分是算术;不是算术的是<em>量级差</em>:
单价从 p10 到 p90 只差 2.9 倍,取餐距离差 20 倍。分子没有变化空间,分母全是。

两个拟合支撑起计算器:

- **派单页在给什么定价** —— `金额 ≈ $3.05 + $0.47/km + $0.203/min`(R² 0.68)。
  时间那一项几乎是零,所以堵车、慢区、等餐全部由骑手自己承担。
- **一单要花多少分钟** —— `分钟 ≈ 3.53 + 4.24 × 总公里`(R² 0.82),即门到门 14.2 km/h
  加约 3.5 分钟固定开销(等餐中位 2.3 分钟 + 交货)。R² 这么高,是计算器敢只用三个滑块的原因。

反过来解就是接单线:`要给的钱 ≥ 目标时薪 × 分钟 / 60`。页面上是一个滑块计算器,
背景是 143 单真实记录的散点,颜色为实测时薪 —— 线画在数据上,可以自己对。

**拼单的价值不在它给的钱里。**金额、距离、计价时长都正好翻倍,派单页上时薪一模一样
($27.7 vs $27.5);唯独骑去餐厅那一段没有翻倍(2.08 km vs 1.94 km)。一次接近服务两户,
实际时薪就从 $18.1 变成 $21.3。

**落点的账记在下一单头上。**落点每远离餐厅群 1 公里,之后的空驶多约 0.9 分钟
(128 段空驶,ρ +0.26)。本来想按 suburb 分开算「送到哪个区最划算」,但每区只摊得到
六七单有价格的记录,区间差距小于噪声,那张表没有出现在页面上;换成连续变量问,
就能用上全部 128 段 —— 包括另外两个平台的,因为码表不认得单是哪个 app 派的。

**这份数据答不上来的问题。**日志里只有接下的单,没有拒掉的单。所以「门槛定在哪一档」
那张表给的是<em>上下界</em>:拒掉一单省下的时间,要么被下一单填上,要么白白空着,
真实情况在两者之间。能说的只是送完之后真正站着不动的时间中位只有半分钟,
说明派单密度不低,往「填得上」那一侧靠。

价格只有一个平台的,时间不是 —— 时间来自码表。143 单同时有价格和完整轨迹。
和上一节一样,客户地址一个字都没有进入 `offers`:散点只带公里数、金额、分钟数和小时。
